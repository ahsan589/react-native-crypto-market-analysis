...
const PriceAlertScreen = () => {
  ...
  const checkPriceAlerts = useCallback(async () => {
    ...
    try {
      const response = await axios.get("https://api.coingecko.com/api/v3/coins/markets", {
        params: {
          vs_currency: "usd",
          ids: alerts.map(alert => alert.coinId).join(','),
          per_page: alerts.length
        }
      });
      ...
      for (const alert of alerts) {
        const coin = currentPrices.find((c: Coin) => c.id === alert.coinId);
        if (!coin) continue;
        ...
        if (shouldTrigger) {
          await Notifications.scheduleNotificationAsync({
            content: {
              ...
              body: `Price is now $${coin.current_price.toFixed(2)} (Target: ${alert.alertType} $${alert.targetPrice.toFixed(2)})`,
              sound: 'default'
            },
            trigger: null
          });
      ...
      if (triggeredAlerts.length > 0) {
        const updatedAlerts = alerts.filter(alert => !triggeredAlerts.includes(alert.id));
  ...
  const setPriceAlert = () => {
    ...
    Notifications.scheduleNotificationAsync({
      content: {
        ...
        body: `Alert set for ${selected.name} (${selected.symbol.toUpperCase()})`,
        sound: 'default'
      },
      trigger: null
    });
  ...
  const removeAlert = (id: string) => {
    ...
    Notifications.scheduleNotificationAsync({
      content: {
        title: 'Alert Removed',
        body: 'Price alert removed successfully',
        sound: 'default'
      },
      trigger: null
    });
          ...
          <View style={styles.alertTypeButtons}>
            ...
            >
              <Icon 
                name="arrow-upward" 
                size={16} 
                color={alertType === 'Above' ? '#fff' : '#03A9F4'} 
            ...
            >
              <Icon 
                name="arrow-downward" 
                size={16} 
                color={alertType === 'Below' ? '#fff' : '#03A9F4'} 
                style={styles.alertTypeIcon} 
              />
              <Text style={[
                styles.alertTypeButtonText,
